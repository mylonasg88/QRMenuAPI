...
describe('/chickens', function(){
    ...
    describe('Find', function(){
        ...
        it('should populate data of a reference for single data', function(done){
            agent
            .get('/chickens/'+oneId2+'?populate=toPop')
            .set('x-tag',tag)
            .then(function(resp){
        ...
        it('should load next page for pagination', function(done){
            agent
            .get('/chickens?lastId='+last+'')
            .set('x-tag',tag)
            .expect(200,done);
        });
        it('should filter by date range', function(done){
            agent
            .get('/chickens?from='+from+'&to='+new Date().toISOString())
            .set('x-tag',tag)
            .then(function(resp){
                resp.body.data.length.should.be.above(0);
                done();
...
describe('Restore', function(){
    it('should restore a previously deleted data', function(done){
        ...
        setTimeout(function(){
            ...
            .then(function(resp){
                return agent
                .post('/chickens/'+resp._id+'/restore')
                .set('x-tag',tag);
            })
            .then(function(resp){
                resp.body.data.should.be.an('object');
