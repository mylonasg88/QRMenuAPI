...
describe('/hello worlds', function(){
    ...
    describe('Find', function(){
        ...
        it('should populate data of a reference for single data', function(done){
            agent
            .get('/hello worlds/'+oneId2+'?populate=toPop')
            .set('x-tag',tag)
            .then(function(resp){
        ...
        it('should load next page for pagination', function(done){
            agent
            .get('/hello worlds?lastId='+last+'')
            .set('x-tag',tag)
            .expect(200,done);
        });
        it('should filter by date range', function(done){
            agent
            .get('/hello worlds?from='+from+'&to='+new Date().toISOString())
            .set('x-tag',tag)
            .then(function(resp){
                resp.body.data.length.should.be.above(0);
                done();
...
describe('Delete', function(){
    ...
    it('should delete multiple data', function(done){
        agent
        .delete('/hello worlds?name=femi2')
        .set('x-tag',tag)
        .then(function(resp){
...
describe('Restore', function(){
    it('should restore a previously deleted data', function(done){
        ...
        setTimeout(function(){
            ...
            .then(function(resp){
                return agent
                .post('/hello worlds/'+resp._id+'/restore')
                .set('x-tag',tag);
            })
            .then(function(resp){
                resp.body.data.should.be.an('object');
