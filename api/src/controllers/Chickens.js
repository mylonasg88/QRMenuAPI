...
var ChickensController = {};

ChickensController.find = function (req, res, next) {
  var query;
  if (req.query.search) {
    ...
    if (projection) {
      ourProjection = ChickensController.buildProjection(projection);
      delete query.select;
    }
    ...
    var totalResult = Chickens.estimatedDocumentCount(query);
    var total = 0; //Chickens.estimatedDocumentCount({});
    var question = Chickens.find(query);
...
ChickensController.findOne = function (req, res, next) {
  ...
  var question = Chickens.findById(id);
  if (populate) {
    delete query.populate;
    question = question.populate(populate);
  }
...
ChickensController.create = function (req, res, next) {
  var data = req.body;
  if (data && data.secure) {
    delete data.secure;
  }
  Chickens.create(data)
    .then(function (resp) {
      res.ok(resp);
    })
    .catch(function (err) {
...
ChickensController.update = function (req, res, next) {
  var query = req.query;
  // Clean appId and userId
  if (query && query.appId) {
    delete query.appId;
  ...
  Chickens.updateMany(query, data)
    .then(function (resp) {
      res.ok(resp);
    })
    .catch(function (err) {
...
ChickensController.updateOne = function (req, res, next) {
  var id = req.params.id;
  var data = req.body;
  if (data && data.secure) {
    delete data.secure;
  ...
  Chickens.findByIdAndUpdate(id, data)
    .then(function (resp) {
      if (!resp) {
        next();
      } else {
...
ChickensController.delete = function (req, res, next) {
  var query = req.query;
  // Clean appId and userId
  if (query && query.appId) {
    delete query.appId;
  ...
  Chickens.find(query)
    ...
    .then(function (resp) {
      // Delete matches
      return [Chickens.deleteMany(query), resp];
    })
...
ChickensController.deleteOne = function (req, res, next) {
  var id = req.params.id;
  // Find match
  Chickens.findById(id)
    .then(function (resp) {
      // Backup data in Trash
      var backupData = {};
      backupData.service = service;
    ...
    .then(function (resp) {
      // Delete match
      return [Chickens.findByIdAndRemove(id), resp];
    })
    .spread(function (deleted, resp) {
      if (!resp) {
        next();
...
ChickensController.restore = function (req, res, next) {
  var id = req.params.id;
  // Find data by ID from trash
  Trash.findById(id)
    .then(function (resp) {
      // Restore to DB
      return Chickens.create(resp.data);
    })
    .then(function (resp) {
      // Delete from trash
...
module.exports = ChickensController;
