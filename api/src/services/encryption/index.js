...
module.exports = {
  generateKey: function () {
    return q.Promise(function (resolve, reject) {
      ...
      crypto.pbkdf2(config.secret, salt, 100000, bits / 8, "sha512", function (
        err,
        key
      ) {
        if (err) {
          ...
          resolve(
            Buffer.from(
              aesjs.utils.hex.fromBytes(key) + "//////" + randomNumber
            ).toString("base64")
          );
        }
      });
  ...
  encrypt: function (text, key) {
    ...
    var truth = crypto.createHash("sha512").update(text).digest("hex");
    return q.Promise(function (resolve) {
      // Convert text to bytes

      var textBytes = aesjs.utils.utf8.toBytes(text);

      var aesCbc = new aesjs.ModeOfOperation.ctr(
        key,
  ...
  decrypt: function (text, key, truthHash) {
    ...
    return q.Promise(function (resolve, reject, notify) {
      ...
      var truthConfirmationHash = crypto
        .createHash("sha512")
    ...
    if (req.get("x-tag") || req.query["x-tag"]) {
      ...
      if (
        ...
        if (req.body.secureData) {
          ...
          encryption
            .decrypt(req.body.secureData, key, truthHash)
            .then(function (resp) {
          ...
          res.badRequest(
            false,
            "Expecting an encrypted data to be sent in the secureData body parameter."
          );
        }
      } else {
        next();
      ...
      res.badRequest(
        false,
        "Please initialize and send the x-tag header in every request."
      );
    }
  },
};
